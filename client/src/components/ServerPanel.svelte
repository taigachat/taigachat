<script lang="ts">
import { setServerViewedRoomID, setActiveVoiceChannel, setServerConnectivityInfo } from '../store'
...
export let profilesURL: (firstName: string) => URL
...
<div class="server-info panel-glass panel-border">
    {#if !server.connectivityInfo.enabled}
    ...
    <div class="server-room-list">
        {#each shownRooms as [roomID, room]}
            <div
                on:click={() => setServerViewedRoomID(server.serverID, parseInt(roomID))}
        ...
        {#each server.channels as channel}
            <div class="list-entry" on:click={joinChannel(channel.channelID)}>
    ...
    <div>
        {#if activeVoiceServer !== undefined}
            {#if voice.activeChannelID !== -1}
                <VoiceControls {voice} channelName={activeVoiceServer.channels[voice.activeChannelID]?.name || 'Unknown name'} />
            {/if}
        {/if}
    </div>
</div>
