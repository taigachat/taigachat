...
let peerToProducers: Map<number, string[]> = new Map()

let mediaElementsParent: HTMLElement|undefined = undefined
let remoteProducerToElement: Map<string, HTMLMediaElement> = new Map()
...
let typeToProducerID: Map<UnspecificMedia, string> = new Map()

interface ConnectCallback {
    connect: () => void,
    err: (e: Error) => void,
}
let connectCallbacks: Map<number, ConnectCallback> = new Map()

interface ProduceCallback {
    produce: (i: {id: string}) => void,
    err: (e: Error) => void,
}
let produceCallbacks: Map<number, ProduceCallback> = new Map()
...
let producers: Map<string, Producer> = new Map()
...
export function setActiveChannel(serverID: number,
                                 channelID: number,
...
function removeConsumer(id: string) {
    let elem = remoteProducerToElement.get(id)
    if (elem !== undefined) {
...
async function producerConsumed(consumer: Consumer, kind: MediaKind, stream: MediaStream) {
    ...
    //; (window as any).debugGetAudioTracks = () => stream.getAudioTracks
    ...
    if (kind === 'video') {
        ...
        elem.volume = 0
        ; (elem as any).playsInline = false // TODO: Remove?
        elem.autoplay = true
        if (mediaElementsParent) {
            mediaElementsParent.appendChild(elem)
        }
...
function closeProducer(type: UnspecificMedia) {
    let producerID = typeToProducerID.get(type)
...
async function setupAudioDetection(userMedia: MediaStream) {
    if (inputAudioContext && inputAudioContext.state === 'running') {
        await inputAudioContext.audioWorklet.addModule('./static/audio_detector.mjs')
    }
...
async function produceMedia(device: MediaDeviceInfoJSON|UnspecificMedia) {
    ...
    const type = (typeof device === 'number')
        ? device
        : (device.kind === 'videoinput' ? UnspecificMedia.VIDEO : UnspecificMedia.AUDIO)
    ...
    const stream = device === UnspecificMedia.SCREEN
    ...
    try {
        ...
        const producer = await producerTransport.produce({
            track,
            encodings: type === UnspecificMedia.VIDEO ? [
                {
                    rid: 'r0',
                    maxBitrate: 100000,
            ...
            codecOptions: type === UnspecificMedia.VIDEO ? {
...
; (window as any).debugProduceMedia = produceMedia

export async function handleSFUMessage(serverID: number, message: MessageFromSFU) {
...
export async function updateCallVolumes(voice: VoiceConfig, activeChannel: Immutable<ActiveChannel>, audioLevels: AudioLevels, pushToTalk: boolean) {
