...
function callUpdateCallVolumes() {
    ...
    updateCallVolumes(previousVoice,
                      previousActiveVoiceChannel,
                      previousAudioLevels,
                      pushToTalk)
}
...
function talkingStateChanged(talking: boolean) {
    ...
    lastTalking = talking
    if (previousVoice
        && (previousVoice.selfMute
            || (pushToTalk
                && !previousVoice.pushToTalkPressed
                && talking))) {
        return
    }
...
function playJoinOrLeaveSound(old: Immutable<ActiveChannel>|undefined,
                              newChannel: Immutable<ActiveChannel>) {
    if (old === undefined) {
        return
    }
    const userDifference = newChannel.connectedUsers.length
                           - old.connectedUsers.length
    if (userDifference > 0) {
        playSound('userJoin')
...
export function handleStoreChanged(s: Immutable<MainStore>) {
    ...
    if (previousVoice !== s.voice) {
        ...
        if (currentServerID !== s.voice.activeServerID
            && currentServerID !== -1) {
        ...
        if (currentServerID !== s.voice.activeServerID
            ...
            setActiveChannel(serverID, currentChannelID, async (m) => {
                onServer.sendMessageSFU0(serverID, m)
            }, callUpdateCallVolumes, talkingStateChanged)
