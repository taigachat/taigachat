...
import { handleStoreChanged as handleStoreChangedRouterBehaviour } from "./router_behaviour";

export function dispatchSubscribeAll() {
    mainStore.setUpdateHandler(async (s) => {
        //console.log('handlers have been called', s)
        await handleStoreChangedActs(s);
        await handleStoreChangedNetCode(s);
        handleStoreChangedRouterBehaviour(s);
