...
// Uses cryptographically secure sources to create a random base64url.
export function randomBase64(length: number): string {
    const source = crypto.getRandomValues(new Uint8Array(length));
    let result = "";
    for (let i = 0; i < length; i++) {
        result += chars[source[i]! % chars.length];
    }
    return result;
}

registerDebugCommand("randomBase64", randomBase64);
registerDebugCommand("debugToBase64", toBase64);
registerDebugCommand("fromBase64", fromBase64);
