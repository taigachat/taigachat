import { toBase64 } from "./encoding_schemes.js";
...
export function getAttachmentUpload(name: string): Promise<string> {
    ...
    return presignS3URL("PUT", "attachments", name, 60 * 60 * 24);
}

export function getProfileUpload(userID: string, profileTimestamp: number): Promise<string> {
    return presignS3URL("PUT", "profiles", `${userID}/${profileTimestamp}.png`, 60 * 60);
}
