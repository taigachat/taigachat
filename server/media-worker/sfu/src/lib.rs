...
async fn process_client_command(channel: &mut Channel, peer_id: PeerID, message: FromClient, tx: &UnboundedSender<IncomingMessage>) -> Result<ToClient> {
    Ok(match message {
        FromClient::CreateTransport{rtp_capabilities, force_tcp, errand} => {
            ...
            let transport = channel.router.create_webrtc_transport(WebRtcTransportOptions::new(TransportListenIps::new(
                ListenIp {
                    ip: channel.listen_ip,
                    announced_ip: Some(channel.announce_ip)
                }
            ))).await?;
...
#[cfg(not(windows))]
async fn start_unixsocket(address: String, tx: UnboundedSender<IncomingMessage>) -> Box<ResponseSender> {
    use futures_util::StreamExt;
    let code = std::env::var("SFU_WORKER_CODE")
        .expect("SFU_CONTROLLER_CODE missing from env");
    let id: usize = std::env::var("SFU_WORKER_ID")
        .expect("SFU_WORKER_ID missing from env")
        .parse()
        .unwrap();

    let socket = tokio::net::UnixStream::connect(address).await.unwrap();

    let codec = tokio_util::codec::LinesCodec::new();
    let parts = tokio_util::codec::FramedParts::new::<String>(socket, codec);
    let framed = tokio_util::codec::Framed::from_parts(parts);
    let (write, mut read) = framed.split();
    let mut write_mapped = write
        .sink_map_err(|e: tokio_util::codec::LinesCodecError|anyhow::anyhow!(e));
    ...
    tokio::spawn(async move {
        while let Some(message) = read.next().await {
            ...
            let message = match serde_json::from_str::<IncomingMessage>(&msg) {
                Ok(m) => m,
                Err(e) => { error!("bad message: {}", e); continue }
            };
    ...
    write_mapped.send(serde_json::to_string(&serde_json::json!({
        "id": id,
        "code": code
    })).unwrap()).await.unwrap();
...
#[cfg(windows)]
async fn start_unixsocket(_address: String, _tx: UnboundedSender<IncomingMessage>) -> Box<ResponseSender> {
    panic!("attempting to listen to a unix socket on a platform \
            that does not support doing so via tokio");
}
...
async fn start_websocket(tx: UnboundedSender<IncomingMessage>) -> Box<ResponseSender> {
    ...
    let (ws_stream, _) = tokio_tungstenite::connect_async(url).await.expect("SFU controller connection failed");
    let (write, mut read) = ws_stream.split();
    let write_mapped = write
        .with(|data| futures_util::future::ok(Message::text(data)))
...
async fn start_websocket(_tx: UnboundedSender<IncomingMessage>) -> Box<ResponseSender> {
    panic!("attempting to listen to WS server without \
            tokio_tungstenite feature enabled");
}
...
pub async fn start_worker() {
    ...
    worker_settings.rtc_ports_range = std::ops::RangeInclusive::new(min_port, max_port);
    ...
    let socket_var = if cfg!(win32) { None } else {
        // Only read this on non-windows.
        std::env::var("SFU_CONTROLLER_SOCKET")
            .ok().filter(|path|!path.is_empty())
    };

    let mut write = if let Some(address) = socket_var {
        start_unixsocket(address, tx.clone()).await
    } else if cfg!(feature = "tokio-tungstenite") {
        start_websocket(tx.clone()).await
    } else {
        panic!("no way to contact SFU controller, \
                please enable tokio-tungstenite feature");
    };
